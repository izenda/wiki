...
Allows user to input the name of the joined table that you selected. By default, Join Alias Textbox is disabled. You have the option to change the Join Alias and the joined table will appear with the alias in Fields Tab when you choose columns. For more information, refer to [Join Alias Textbox](http://wiki.izenda.us/API/CodeSamples/ShowJoinAliasTextboxes)
